...
tl.add([tween1, tween2, tween3], "+=2", "sequence", 0.5);
		...
		/*Custom function that adds the contents of a vector containing timelines. Meant to increase performance by avoiding use of 
		 * untyped objects and investigation of types*/
		public function AddVectorOfTimelines(timelines:Vector.<TimelineLite>):void
		{
			var parentTime:Number = this._time;
			for (var i:int = 0, l:int = timelines.length; i < l; ++i)
			{
				var timelineAdding:TimelineLite = timelines[i];
				timelineAdding._startTime = 0 + timelineAdding._delay;
				if (timelineAdding._paused) if (this != timelineAdding._timeline) { //we only adjust the _pauseTime if it wasn't in this timeline already. Remember, sometimes a tween will be inserted again into the same timeline when its startTime is changed so that the tweens in the TimelineLite/Max are re-ordered properly in the linked list (so everything renders in the proper order). 
					timelineAdding._pauseTime = timelineAdding._startTime + ((rawTime() - timelineAdding._startTime) / timelineAdding._timeScale);
				}
				if (timelineAdding.timeline) {
					timelineAdding.timeline._remove(timelineAdding, true); //removes from existing timeline so that it can be properly added to this one.
				}
				timelineAdding.timeline = timelineAdding._timeline = this;
				if (timelineAdding._gc) {
					timelineAdding._enabled(true, true);
				}
				
				var prevTween:Animation = _last;
				if (_sortChildren) {
					var st:Number = timelineAdding._startTime;
					while (prevTween && prevTween._startTime > st) {
						prevTween = prevTween._prev;
					}
				}
				if (prevTween) {
					timelineAdding._next = prevTween._next;
					prevTween._next = Animation(timelineAdding);
				} else {
					timelineAdding._next = _first;
					_first = Animation(timelineAdding);
				}
				if (timelineAdding._next) {
					timelineAdding._next._prev = timelineAdding;
				} else {
					_last = Animation(timelineAdding);
				}
				timelineAdding._prev = prevTween;
				
				
				//timelineAdding._time = parentTime;
				timelineAdding.seek(0);
				timelineAdding.play(parentTime);
			...
			if (_gc || _time === _duration) if (!_paused) if (_duration < duration()) {
				//in case any of the anscestors had completed but should now be enabled...
				var tl:SimpleTimeline = this,
					beforeRawTime:Boolean = (tl.rawTime() > timelineAdding._startTime); //if the tween is placed on the timeline so that it starts BEFORE the current rawTime, we should align the playhead (move the timeline). This is because sometimes users will create a timeline, let it finish, and much later append a tween and expect it to run instead of jumping to its end state. While technically one could argue that it should jump to its end state, that's not what users intuitively expect.
				while (tl._timeline) 
				{
					if (beforeRawTime && tl._timeline.smoothChildTiming) {
						tl.totalTime(tl._totalTime, true); //moves the timeline (shifts its startTime) if necessary, and also enables it.
		...
		public function RemoveTimelines(timelines:Vector.<TimelineLite>):void
		{
			var timelineRemoving:TimelineLite;
			for (var i:int = 0, l:int = timelines.length; i < l; ++i)
			{
				timelineRemoving = timelines[i];
				timelineRemoving.stop();
				if (timelineRemoving.timeline == this) {
					//if (!skipDisable) {
					
					timelineRemoving._active = false;
						//timelineRemoving._enabled(false, true);
					}//
					
					if (timelineRemoving._prev) {
						timelineRemoving._prev._next = timelineRemoving._next;
					} else if (_first === timelineRemoving) {
						_first = timelineRemoving._next;
					}
					if (timelineRemoving._next) {
						timelineRemoving._next._prev = timelineRemoving._prev;
					} else if (_last === timelineRemoving) {
						_last = timelineRemoving._prev;
					}
					timelineRemoving._next = timelineRemoving._prev = timelineRemoving.timeline = null;
			}
			if (_timeline) {
				_uncache(true);
			}
